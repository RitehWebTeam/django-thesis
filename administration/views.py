...
from .forms import ThesisForm, VerificationForm, PresidentChangeStatusForm, AddComitee, ChangeDocument, GradeForm, \
    FinalGradeForm
...
from const.Variabels import Variabels
    ...
    **Template:**
    ...
    if request.user.is_authenticated:
        ...
        try:
            ...
            # TODO malo bolje treba ovo napraviti
            pass
        try:
...
def insert_thesis_into_db(request, form, student, thesis_id=None):
    ...
    thesis.mentor = form.cleaned_data.get("mentor")  # Ovo bi se moglo izbaciti u posebnu funkciju?
    ...
    if 'Diplomski' in thesis.study.__str__():
        rad = "DIPLOMSKI"
        thesis.type = 2
        for studij in Variabels.SVEUCILISNI_DIPLOMSKI_STUDIJ:
            if studij['name'] in thesis.study.__str__():
                president_id, deputy_id, writer_id = studij['role_list']
                break
    else:
        thesis.type = 1
        rad = "ZAVRŠNI"
        for studij in Variabels.SVEUCILISNI_PREDDIPLOMSKI_STUDIJ:
            if studij['name'] in thesis.study.__str__():
                president_id, deputy_id, writer_id = studij['role_list']
                break
    ...
    if thesis.status == 2:
        ...
        if thesis.komentor != None:
            ...
            komentor_notification.content = SUBMISSION_CONTENT.format(request.user.id, thesis.author, thesis.id,
                                                                      thesis.title)
    ...
    **Template:**
    ...
    if request.user.is_authenticated:
        ...
        try:
            ...
            if request.method == 'POST':
                form = ThesisForm(request.POST, request.FILES, student=student,
                                  instance=Thesis.objects.get(id=thesis_id))
...
def thesis_display(request, thesis_id):
    ...
    **Template:**
    ...
    if request.user.is_authenticated:
        ...
        try:
            student = request.user.student
            nav = "student/student_index.html"
        except Student.DoesNotExist:
            student = None

        try:
            employee = request.user.employee
            nav = "employee/employee_index.html"
        except Employee.DoesNotExist:
            employee = None
    ...
    **Template:**
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            student_notification.content = STUDENT_VERIFICATION_CONTENT.format(thesis.id, thesis.title,
                                                                               thesis.verification_of_authenticity)
            student_notification.save()

            mentor_notification = Notification(user=thesis.mentor.user)
            mentor_notification.title = VERIFICATION_TITLE
            mentor_notification.content = MENTOR_VERIFICATION_CONTENT.format(thesis.id, thesis.title,
                                                                             thesis.verification_of_authenticity)
            mentor_notification.save()
            return HttpResponseRedirect(request.META.get('HTTP_REFERER', '/'))
    return render(request, 'error.html')
...
def mentor_status_change(request):
    ...
    **Template:**
    ...
    if request.method == "POST":
        ...
        student_notification.content = STATUS_CHANGED_CONTENT.format(thesis.id, thesis.title,
                                                                     thesis.get_status_display())
        student_notification.save()

        mentor_notification = Notification(user=thesis.mentor.user)
        mentor_notification.title = STATUS_CHANGE_TITLE
        mentor_notification.content = STATUS_CHANGER_CONTENT.format(thesis.get_status_display(), thesis.id,
                                                                    thesis.title)
        mentor_notification.save()
        return HttpResponseRedirect(request.META.get('HTTP_REFERER', '/'))
    return render(request, 'error.html')
    ...
    **Template:**
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            president_notification.content = STATUS_CHANGER_CONTENT.format(thesis.get_status_display(), thesis.id,
                                                                           thesis.title)
...
def student_status_change(request):
    ...
    **Template:**
    ...
    if request.method == "POST":
        ...
        president_notification.content = SUBMISSION_CONTENT.format(thesis.author.user.id, thesis.author.user, thesis.id,
                                                                   thesis.title)
        president_notification.save()
        ...
        deputy_notification.content = SUBMISSION_CONTENT.format(thesis.author.user.id, thesis.author.user, thesis.id,
                                                                thesis.title)
        ...
        writer_notification.content = SUBMISSION_CONTENT.format(thesis.author.user.id, thesis.author.user, thesis.id,
                                                                thesis.title)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            for i in MemberOfThesisComitee.objects.filter(thesis=thesis):
                ...
                student_notification.content += '<a href="/profil/{}/">{}</a> <br>'.format(i.employee.user.id,
                                                                                           i.employee)
            student_notification.save()
            return HttpResponseRedirect(request.META.get('HTTP_REFERER', '/'))
        else:
    ...
    **Template:**
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            member_notification.content = COMITEE_MEMBER_ADDED_GRADE_CONTENT.format(thesis.id, thesis.title,
                                                                                    member.grade, member.defense_grade)
            member_notification.save()
            grade = 0
            defense_grade = 0
            final_grade = 0
            if not MemberOfThesisComitee.objects.filter(grade=None, defense_grade=None, thesis=thesis):
                ...
                student_notification.content = STUDENT_MASTER_GRADE_CONTENT.format(thesis.id, thesis.title,
                                                                                   thesis.final_grade,
                                                                                   thesis.get_status_display())
                for i in MemberOfThesisComitee.objects.filter(thesis=thesis):
                    student_notification.content += '<a href="/profil/{}/">{}</a> : Ocjena rada: {}, Ocjena obrane: {} <br>'.format(
                        i.employee.user.id, i.employee, i.grade, i.defense_grade)
                student_notification.save()
    ...
    **Template:**
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            student_notification.content = STUDENT_BACHELOR_GRADE_CONTENT.format(thesis.id, thesis.title,
                                                                                 thesis.final_grade,
                                                                                 thesis.get_status_display())
            student_notification.save()
            ...
            mentor_notification.content = MENTOR_GRADE_CONTENT.format(thesis.id, thesis.title, thesis.final_grade,
                                                                      thesis.get_status_display())
            mentor_notification.save()
